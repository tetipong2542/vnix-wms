...
def create_app():
    ...
    def dashboard():
        ...
        if is_all_time:
            ...
            # [แก้ไขล่าสุด] Default View -> ให้แสดงยอดจ่ายงาน "เฉพาะวันนี้" เท่านั้น
            # เพื่อให้ข้ามวันแล้วรีเซ็ตเป็น 0
    ...
    def dashboard_issued():
        ...
        # [แก้ไข] รับค่าพารามิเตอร์แบบดิบๆ ก่อน (ยังไม่แปลงเป็น "")
        date_from_param = request.args.get("date_from")
        date_to_param = request.args.get("date_to")

        # ตรวจสอบว่าเป็น Default Load หรือไม่ (ไม่มีพารามิเตอร์ส่งมาเลย และไม่ได้ค้นหา)
        if date_from_param is None and date_to_param is None and not q:
            # ถ้าใช่ -> บังคับให้เป็น "วันนี้"
            today_str = now_thai().date().isoformat()
            date_from_str = today_str
            date_to_str = today_str
        else:
            # ถ้ามีการส่งค่ามา (เช่น กดกรอง, ค้นหา, หรือลบวันที่ออก) ให้ใช้ตามที่ส่ง
            date_from_str = date_from_param or ""
            date_to_str = date_to_param or ""
