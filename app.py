...
def create_app():
    ...
    class ImportLog(db.Model):
        ...
        # แก้ไขให้ใช้ Thai timezone (GMT+7) แทน UTC
    ...
    def _orders_ready_set(rows: list[dict]) -> set[str]:
        ...
        for oid, items in by_oid.items():
            ...
            for it in items:
                ...
                # [แก้ไข] รองรับ BILL_EMPTY: Order ที่เป็นบิลเปล่าก็ถือว่าพร้อมรับได้
        ...
        for oid, items in by_oid.items():
            ...
            for it in items:
                ...
                if packed or accepted or is_issued:
                    all_sendable = False
                    break
                # [แก้ไข] รองรับ BILL_EMPTY: BILL_EMPTY orders ที่มี LOW_STOCK items ก็สามารถอยู่กอง 2 ได้
    ...
    def _mark_printed(oids: list[str], kind: str, user_id: int | None, when_iso: str, commit: bool = True):
    ...
    def admin_shops():
        ...
        # [แก้ไข] กรองเฉพาะร้านค้าจริง ไม่รวมระบบ config (SYSTEM platforms)
        system_platforms = ['EMPTY_BILL_SYSTEM', 'PRODUCTS_SYSTEM', 'SALES_SYSTEM', 'STOCK_SYSTEM']
        shops = Shop.query.filter(~Shop.platform.in_(system_platforms)).order_by(Shop.platform.asc(), Shop.name.asc()).all()
    ...
    @login_required
    def price_dashboard():
        return render_template("price_dashboard.html", APP_NAME=APP_NAME)
    ...
    def dashboard():
        ...
        def _build_card_tags(oid: str) -> list:
            ...
            # ตรวจสอบว่า Order นี้เป็น BILL_EMPTY หรือไม่
            is_bill_empty = oid in kpi_orders_bill_empty

            # กลุ่มงานค้าง (Pending Tasks)
            # ถ้าเป็น BILL_EMPTY ให้ตรวจสอบว่าอยู่กองไหนด้วย (เพื่อแสดง badge กอง)
            if oid in kpi_orders_ready:
                tags.append({"code": "ORDER_READY", "label": "กอง 1", "cls": "bg-success"})
            elif is_bill_empty and oid not in kpi_orders_low and oid not in kpi_orders_problem:
                # ถ้าเป็น BILL_EMPTY และไม่อยู่ในกอง 2/3 แสดงว่าอยู่กอง 1
                tags.append({"code": "ORDER_READY", "label": "กอง 1", "cls": "bg-success"})
            ...
            # แสดง badge บิลเปล่าเสมอถ้าเป็น BILL_EMPTY
            if is_bill_empty:
                tags.append({"code": "BILL_EMPTY", "label": "บิลเปล่า", "cls": "bg-purple text-white"})
    ...
    def export_excel():
        ...
        for r in rows:
            ...
            elif st == "BILL_EMPTY": st_display = "บิลเปล่า"
            else: st_display = st
    ...
    def admin_clear():
        ...
        stats = {
            ...
            # [แก้ไข] แสดงเฉพาะจำนวนบิลที่สำเร็จ (added_count) ไม่ใช่จำนวนครั้งที่ import
            "bill_empty_today": db.session.query(func.sum(ImportLog.added_count)).filter(
                ImportLog.platform == 'EMPTY_BILL_SYSTEM',
                ImportLog.import_date == today
            ).scalar() or 0,
        }
            ...
            try:
                ...
                if lines:
                    ...
                    is_already_bill_empty = any(
                        db.session.execute(
                            text("SELECT allocation_status FROM order_lines WHERE id = :id"),
                            {"id": line.id}
                        ).scalar() == 'BILL_EMPTY'
                        for line in lines
                    )
                    ...
                    for line in lines:
                        db.session.execute(
                            text("UPDATE order_lines SET allocation_status = 'BILL_EMPTY' WHERE id = :id"),
                            {"id": line.id}
                        )
    ...
    def import_bill_empty_view():
                ...
                try:
                    ...
                    if sheet_url:
                        try:
                            ...
                            if not config_shop:
                                config_shop = Shop(platform='EMPTY_BILL_SYSTEM', name='GoogleSheet')
        ...
        # คำนวณสถิติจาก Logs
        total_success = sum(log.added_count for log in logs)
        ...
        today_date = now_thai().date()
        success_old = sum(log.added_count for log in logs if log.import_date < today_date)
        success_new = sum(log.added_count for log in logs if log.import_date >= today_date)
        duplicates_old = sum(log.duplicates_count for log in logs if log.import_date < today_date)
        duplicates_new = sum(log.duplicates_count for log in logs if log.import_date >= today_date)
        failed_old = sum(log.failed_count for log in logs if log.import_date < today_date)
        failed_new = sum(log.failed_count for log in logs if log.import_date >= today_date)
