...
def import_sales(df: pd.DataFrame) -> list[str]:  # <--- เปลี่ยน Return Type เป็น list
    """
    นำเข้าข้อมูลใบสั่งขาย (Sales)
    Returns: List ของ Order ID ที่ทำการ Create/Update สำเร็จ
    """
    # 1. หาชื่อคอลัมน์
    ...
    processed_ids = []  # <--- ตัวแปรเก็บรายชื่อ ID ที่ทำสำเร็จ

    # 2. แปลงข้อมูลให้สะอาด
    for _, row in df.iterrows():
        oid = str(row.get(col_oid, "")).strip()
    ...
    db.session.commit()
    return processed_ids  # <--- ส่งคืน List รายชื่อ ID
